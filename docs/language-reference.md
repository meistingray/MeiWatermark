...
| 已导出 {count} 张图片。 | Exported {count} images. | Se exportaron {count} imágenes. | {count} 枚の画像を書き出しました。 |
| 失败 {count} 张： | Failed: {count} | Fallidas: {count} | 失敗: {count} 枚 |
| 导出 {current}/{total}: {name} | Exporting {current}/{total}: {name} | Exportando {current}/{total}: {name} | 書き出し {current}/{total}: {name} |
