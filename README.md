...
MeiWatermark 是一款便携、本地运行的 Windows 批量图片水印工具，支持图片水印、文字水印、全屏水印、多图层和批量导出。
...
- **实时预览**：拖入照片后异步加载；加载下一张时仍保留当前预览，直到新照片完成加载；本次新增的照片会自动选中并滚动到列表可见位置。
- **渲染保护**：px 水印尺寸超过 4096 会自动调整为 4096；百分比水印实际换算后或平铺数量超过安全范围时会提示并跳过该图片。
- **照片列表管理**：支持 Delete 键、右键菜单移除照片，以及清空列表；这些操作不会删除磁盘中的原始文件。
- **批量导出**：支持 JPEG、PNG、WebP；质量滑杆默认为 100，并可按最长边、最短边或比例限制尺寸，也可以不限制尺寸。
- **输出控制**：导出前估算当前照片文件大小；可保留 EXIF 和 ICC 配置文件；支持相对于每张原图的输出路径。
